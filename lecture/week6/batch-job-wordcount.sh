...
#SBATCH --account=siads618f22_class        # change to your account
#SBATCH --partition=standard
#SBATCH --nodes=2                # node count, change as needed
#SBATCH --ntasks-per-node=1      # do not change, leave as 1 task per node
#SBATCH --cpus-per-task=6       # cpu-cores per task, change as needed
#SBATCH --mem=60g               # memory per node, change as needed
...
spark-submit --master ${SPARK_MASTER_URL} \
  ...
  /sw/examples/spark/spark-on-hpc/examples/word-count.py  "/nfs/turbo/arcts-data-hadoop-stage/data/Gutenberg.txt"
